...
const baseQueryLastidRequest: object = {};

export const QueryLastidRequest = {
  encode(_: QueryLastidRequest, writer: Writer = Writer.create()): Writer {
    return writer;
  },
  ...
  decode(input: Reader | Uint8Array, length?: number): QueryLastidRequest {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseQueryLastidRequest } as QueryLastidRequest;
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        default:
...
const baseQueryLastidResponse: object = { lastid: "" };
...
export const QueryLastidResponse = {
  ...
  decode(input: Reader | Uint8Array, length?: number): QueryLastidResponse {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseQueryLastidResponse } as QueryLastidResponse;
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
...
export class QueryClientImpl implements Query {
  ...
  Params(request: QueryParamsRequest): Promise<QueryParamsResponse> {
    ...
    const promise = this.rpc.request(
      "xmonader.intidgen.intidgen.Query",
      "Params",
      data
    );
